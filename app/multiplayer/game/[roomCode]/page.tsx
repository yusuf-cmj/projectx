...
import { Timer, Film, Gamepad2, Play, Pause, Zap, Users, Award } from 'lucide-react';
...
const Leaderboard: React.FC<LeaderboardProps> = ({ players }) => {
  ...
  return (
    <motion.div 
      layout
      className="w-72 bg-purple-800/40 backdrop-blur-md p-4 rounded-2xl border border-purple-400/20 shadow-xl shadow-purple-500/20 flex flex-col"
    >
      <h3 className="text-xl font-bold text-white mb-4 flex items-center justify-center gap-2 flex-shrink-0">
      ...
      {sortedPlayers.length > 0 ? (
        <motion.ul 
          layout 
          className="space-y-2 overflow-y-auto flex-grow pr-1 custom-scrollbar-players"
        >
          {sortedPlayers.map((player, index) => (
            <motion.li
              ...
              className={`flex items-center justify-between p-3 rounded-lg shadow-md transition-all duration-300 ease-in-out 
                          ${index === 0 ? 'bg-yellow-500/30 border-yellow-400/50' : 'bg-purple-700/50 border-purple-600/50'}
                          hover:shadow-purple-400/30`}
            >
              <div className="flex items-center gap-2">
                <span className={`font-semibold text-sm ${index === 0 ? 'text-yellow-300' : 'text-purple-200'}`}>#{index + 1}</span>
                <span className="text-white font-medium truncate max-w-[120px]">{player.name}</span>
                {index === 0 && <span className="text-xs">👑</span>}
              </div>
              <span className={`font-bold text-lg ${index === 0 ? 'text-yellow-400' : 'text-white'}`}>
        ...
        <div className="flex-grow flex items-center justify-center">
          <p className="text-purple-300 italic">Waiting for players...</p>
        </div>
      )}
    </motion.div>
  );
      ...
      <div className="min-h-screen bg-gradient-to-b from-purple-900 to-indigo-900 flex flex-col items-center justify-center p-4">
        <div className="bg-purple-800/50 backdrop-blur-md p-8 rounded-3xl border border-purple-400/30 shadow-xl shadow-purple-500/30 text-center max-w-lg w-full animate-in fade-in duration-500">
          ...
          <Button
            variant="default"
            size="lg"
            onClick={() => router.push(`/multiplayer/lobby/${roomCode}`)}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-900 to-indigo-900 flex flex-col items-center justify-center px-4 py-6">
      ...
      <div className="flex flex-row gap-6 w-full max-w-5xl">
        ...
        <div className="flex-grow bg-purple-800/40 backdrop-blur-md p-6 rounded-2xl border border-purple-400/20 shadow-xl shadow-purple-500/20 relative animate-in fade-in duration-300">
        ...
        {roomData && roomData.players && <Leaderboard players={roomData.players} />}
      </div>
    </div>
  );
} 